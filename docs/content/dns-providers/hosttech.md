...
lego's provider descriptor lists `HOSTTECH_PASSWORD`, but the Hosttech API client authenticates with the API key alone and has no password field. Only `apiKey` is needed.
