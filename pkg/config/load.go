...
const configFileEnvVar = "LECERTSERVER_CONFIG"

func LoadConfig() error {
	// Get the path to the config.yaml
	// First, try with the LECERTSERVER_CONFIG env var
	...
	if configFile != "" {
		...
		configFile = FindConfigFile("config.yaml", ".", "~/.le-cert-server", "/etc/le-cert-server")
		if configFile == "" {
			// Ok, if you really, really want to use ".yml"....
			configFile = FindConfigFile("config.yml", ".", "~/.le-cert-server", "/etc/le-cert-server")
		}

		// Config file not found
		if configFile == "" {
			return NewConfigError("Could not find a configuration file config.yaml in the current folder, '~/.le-cert-server', or '/etc/le-cert-server'", "Error loading config file")
		}
	}
