// Code generated by gen-dns-providers; DO NOT EDIT.

package config

import (
	"fmt"

	yaml "sigs.k8s.io/yaml/goyaml.v3"
)

// HurricaneConfig holds configuration for the "hurricane" DNS provider (Hurricane Electric DNS)
// See https://dns.he.net/
type HurricaneConfig struct {
	Tokens             string // HURRICANE_TOKENS: TXT record names and tokens
	HTTPTimeout        string // HURRICANE_HTTP_TIMEOUT: API request timeout in seconds (Default: 30)
	PollingInterval    string // HURRICANE_POLLING_INTERVAL: Time between DNS propagation check in seconds (Default: 2)
	PropagationTimeout string // HURRICANE_PROPAGATION_TIMEOUT: Maximum waiting time for DNS propagation (Default: 300)
	SequenceInterval   string // HURRICANE_SEQUENCE_INTERVAL: Time between sequential requests in seconds (Default: 60)
}

// envVars returns the lego environment variables for the populated (non-empty) fields
func (c *HurricaneConfig) envVars() map[string]string {
	m := make(map[string]string, 5)
	if c.Tokens != "" {
		m["HURRICANE_TOKENS"] = c.Tokens
	}
	if c.HTTPTimeout != "" {
		m["HURRICANE_HTTP_TIMEOUT"] = c.HTTPTimeout
	}
	if c.PollingInterval != "" {
		m["HURRICANE_POLLING_INTERVAL"] = c.PollingInterval
	}
	if c.PropagationTimeout != "" {
		m["HURRICANE_PROPAGATION_TIMEOUT"] = c.PropagationTimeout
	}
	if c.SequenceInterval != "" {
		m["HURRICANE_SEQUENCE_INTERVAL"] = c.SequenceInterval
	}
	return m
}

// UnmarshalYAML decodes the provider credentials
// It accepts the normalized name, the raw lego environment variable, and documented aliases; unknown keys error
func (c *HurricaneConfig) UnmarshalYAML(value *yaml.Node) error {
	if value.Kind != yaml.MappingNode {
		return fmt.Errorf("dnsCredentials for DNS provider \"hurricane\" must be a map")
	}
	for i := 0; i+1 < len(value.Content); i += 2 {
		var key string
		err := value.Content[i].Decode(&key)
		if err != nil {
			return err
		}
		var val string
		err = value.Content[i+1].Decode(&val)
		if err != nil {
			return err
		}
		switch key {
		case "tokens", "HURRICANE_TOKENS":
			c.Tokens = val
		case "httpTimeout", "HURRICANE_HTTP_TIMEOUT":
			c.HTTPTimeout = val
		case "pollingInterval", "HURRICANE_POLLING_INTERVAL":
			c.PollingInterval = val
		case "propagationTimeout", "HURRICANE_PROPAGATION_TIMEOUT":
			c.PropagationTimeout = val
		case "sequenceInterval", "HURRICANE_SEQUENCE_INTERVAL":
			c.SequenceInterval = val
		default:
			return fmt.Errorf("unknown credential key %q for DNS provider \"hurricane\"", key)
		}
	}
	return nil
}
