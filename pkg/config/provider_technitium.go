// Code generated by gen-dns-providers; DO NOT EDIT.

package config

import (
	"fmt"
	"strconv"
	"time"

	"github.com/go-acme/lego/v5/challenge"
	prov "github.com/go-acme/lego/v5/providers/dns/technitium"
	yaml "sigs.k8s.io/yaml/goyaml.v3"
)

// TechnitiumConfig holds configuration for the "technitium" DNS provider (Technitium)
// See https://technitium.com/
type TechnitiumConfig struct {
	APIToken           string // TECHNITIUM_API_TOKEN: API token
	ServerBaseURL      string // TECHNITIUM_SERVER_BASE_URL: Server base URL
	PollingInterval    string // TECHNITIUM_POLLING_INTERVAL: Time between DNS propagation check in seconds (Default: 2)
	PropagationTimeout string // TECHNITIUM_PROPAGATION_TIMEOUT: Maximum waiting time for DNS propagation in seconds (Default: 60)
	TTL                string // TECHNITIUM_TTL: The TTL of the TXT record used for the DNS challenge in seconds (Default: 120)
}

// newProvider builds the lego DNS challenge provider using strong types
// Credentials are passed directly to lego and never written to the process environment
func (c *TechnitiumConfig) newProvider() (challenge.Provider, error) {
	cfg := prov.NewDefaultConfig()
	if c.APIToken != "" {
		cfg.APIToken = c.APIToken
	}
	if c.ServerBaseURL != "" {
		cfg.BaseURL = c.ServerBaseURL
	}
	if c.PollingInterval != "" {
		v, err := strconv.Atoi(c.PollingInterval)
		if err != nil {
			return nil, fmt.Errorf("invalid value for \"pollingInterval\": %w", err)
		}
		cfg.PollingInterval = time.Duration(v) * time.Second
	}
	if c.PropagationTimeout != "" {
		v, err := strconv.Atoi(c.PropagationTimeout)
		if err != nil {
			return nil, fmt.Errorf("invalid value for \"propagationTimeout\": %w", err)
		}
		cfg.PropagationTimeout = time.Duration(v) * time.Second
	}
	if c.TTL != "" {
		v, err := strconv.Atoi(c.TTL)
		if err != nil {
			return nil, fmt.Errorf("invalid value for \"ttl\": %w", err)
		}
		cfg.TTL = v
	}
	return prov.NewDNSProviderConfig(cfg)
}

// UnmarshalYAML decodes the provider credentials
// It accepts the normalized name, the raw lego environment variable, and documented aliases
func (c *TechnitiumConfig) UnmarshalYAML(value *yaml.Node) error {
	if value.Kind != yaml.MappingNode {
		return fmt.Errorf("dnsCredentials for DNS provider \"technitium\" must be a map")
	}
	for i := 0; i+1 < len(value.Content); i += 2 {
		var key string
		err := value.Content[i].Decode(&key)
		if err != nil {
			return err
		}
		var val string
		err = value.Content[i+1].Decode(&val)
		if err != nil {
			return err
		}
		switch key {
		case "apiToken", "TECHNITIUM_API_TOKEN":
			c.APIToken = val
		case "serverBaseURL", "TECHNITIUM_SERVER_BASE_URL":
			c.ServerBaseURL = val
		case "pollingInterval", "TECHNITIUM_POLLING_INTERVAL":
			c.PollingInterval = val
		case "propagationTimeout", "TECHNITIUM_PROPAGATION_TIMEOUT":
			c.PropagationTimeout = val
		case "ttl", "TECHNITIUM_TTL":
			c.TTL = val
		default:
			return fmt.Errorf("unknown credential key %q for DNS provider \"technitium\"", key)
		}
	}
	return nil
}
