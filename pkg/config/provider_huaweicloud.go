...
func (c *HuaweicloudConfig) envVars() map[string]string {
	m := make(map[string]string, 7)
	if c.AccessKeyID != "" {
		m["HUAWEICLOUD_ACCESS_KEY_ID"] = c.AccessKeyID
	}
	if c.Region != "" {
		m["HUAWEICLOUD_REGION"] = c.Region
	}
	if c.SecretAccessKey != "" {
		m["HUAWEICLOUD_SECRET_ACCESS_KEY"] = c.SecretAccessKey
	}
	if c.HTTPTimeout != "" {
		m["HUAWEICLOUD_HTTP_TIMEOUT"] = c.HTTPTimeout
	}
	if c.PollingInterval != "" {
		m["HUAWEICLOUD_POLLING_INTERVAL"] = c.PollingInterval
	}
	if c.PropagationTimeout != "" {
		m["HUAWEICLOUD_PROPAGATION_TIMEOUT"] = c.PropagationTimeout
	}
	if c.TTL != "" {
		m["HUAWEICLOUD_TTL"] = c.TTL
	}
	return m
}
